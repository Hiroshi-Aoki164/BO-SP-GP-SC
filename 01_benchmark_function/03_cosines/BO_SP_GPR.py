...
def process_single_seed(seed_num, shared_data):
    try:
        ...
        X_act_expanded = pd.concat([X_act,X_act**2],axis=1)
        X_act_expanded.columns = ["X1","X2","X1^2","X2^2"]
        X_grid_expanded = pd.concat([X_grid,X_grid**2],axis=1)
        X_grid_expanded.columns = ["X1","X2","X1^2","X2^2"]
        ...
        for count in range(0,count_num):
            ...
            X_act_expanded = pd.concat([X_act, X_act**2], axis=1)  # Polynomial terms are assumed known
            X_act_expanded.columns = ["X1","X2","X1^2","X2^2"]
