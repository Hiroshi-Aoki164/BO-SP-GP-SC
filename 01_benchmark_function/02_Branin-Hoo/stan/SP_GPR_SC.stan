...
data {
    int<lower=1> N;
    int<lower=1> D;
    int<lower=1> D_expanded;
    matrix[N,D] X;
    matrix[N,D_expanded] X_expanded;
    vector[N] y;
    int<lower=1> N_grid;
    matrix[N_grid,D] X_grid;
    vector[D_expanded] a_mean;
    vector[D_expanded] a_sd;
    real b_mean;
    real b_sd;
    matrix[N_grid,D_expanded] X_grid_expanded;
    }
...
parameters {
    ...
    real<lower=0> alpha;
    real<lower=0> sd_e;
    real<upper=0> a1; //minus coef.
    real<upper=0> a2; //minus coef.
    real<lower=0> a3; //plus coef.
    real<lower=0> a4; //plus coef.
    real<upper=0> a5; //minus coef.
    real<lower=0> a6; //plus coef.
    real b;
}
...
generated quantities {
  ...
  for (n in 1:N_grid) {
    ...
    mu_star1 = X_grid_expanded[n,]*a + b;
