...
def process_single_seed(seed_num, shared_data):
    """
    Run processing for a single seed (for multiprocessing).
    """
    try:
        ...
        eq = shared_data['eq']
        level = shared_data['level']
        level2 = shared_data['level2']
        ...
        D = len(X_act.T)
        D_expanded = len(X_act_expanded.T)

        a_mean, b_mean = sp.cal_coef(X_act_expanded, y_act)
        a_sd = np.full(D_expanded, 4)
        b_sd = 4
        ...
        for count in range(0,count_num):
            ...
            data = {
                    ...
                    "b_mean" : b_mean,
                    "b_sd" : b_sd,
                    "X_grid_expanded" : X_grid_expanded
              }
            ...
            print(
                ...
                f"count(r_hat <= 1.10) = {sum(summary['r_hat'] <= 1.10)}"
            )
            ...
            sp.plot_contour(X1, X2, z_EI, title, file_pass, level2, X_act, N_first)
            ...
            sp.plot_contour(X1, X2, z_ysd, title, file_pass, level2, X_act, N_first)

            sigma_ave.loc[count,0] = y_sd.mean()[0]
            a1_ave.loc[count,0] = samples["a"][:,0].mean()
            a2_ave.loc[count,0] = samples["a"][:,1].mean()
            a3_ave.loc[count,0] = samples["a"][:,2].mean()
            a4_ave.loc[count,0] = samples["a"][:,3].mean()
            a5_ave.loc[count,0] = samples["a"][:,4].mean()
            a6_ave.loc[count,0] = samples["a"][:,5].mean()
...
def main():
    ...
    a3_ave_all = pd.DataFrame()
    a4_ave_all = pd.DataFrame()
    a5_ave_all = pd.DataFrame()
    a6_ave_all = pd.DataFrame()
    ...
    for seed_num, result in results:
        if result is not None:
            ...
            a3_ave_all = pd.concat([a3_ave_all, result["a3_ave"]], axis=1)
            a4_ave_all = pd.concat([a4_ave_all, result["a4_ave"]], axis=1)
            a5_ave_all = pd.concat([a5_ave_all, result["a5_ave"]], axis=1)
            a6_ave_all = pd.concat([a6_ave_all, result["a6_ave"]], axis=1)
        else:
            print(f"Seed {seed_num}: invalid result")
    ...
    a3_ave_all.to_csv(f_result/"a3.csv", encoding="shift-jis")
    a4_ave_all.to_csv(f_result/"a4.csv", encoding="shift-jis")
    a5_ave_all.to_csv(f_result/"a5.csv", encoding="shift-jis")
    a6_ave_all.to_csv(f_result/"a6.csv", encoding="shift-jis")
