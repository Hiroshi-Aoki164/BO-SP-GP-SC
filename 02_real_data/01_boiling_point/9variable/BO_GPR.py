...
# -*- coding: utf-8 -*-
"""
Parallel execution script for BO_GPR.py (for 10 variables)
...
def process_single_seed(seed_num, shared_data):
    try:
        ...
        for count in range(0,count_num):
            ...
            y_std_pred = samples["y_new"].std(0)
            ...
            y_std_ave.loc[count,0] = y_std_pred.mean()
...
def main():
    ...
    for seed_num, result in results:
        if result is not None:
            ...
            y_std_ave_all = pd.concat([y_std_ave_all, result["y_std_ave"]], axis=1)
        else:
            print(f"Seed {seed_num} result is invalid")

    print("Saving results...")

    y_std_ave_all.to_csv(f_result/"y_std.csv", encoding="shift-jis")
