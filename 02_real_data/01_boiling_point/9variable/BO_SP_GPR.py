...
# -*- coding: utf-8 -*-
"""
Parallel execution script for BO_SP_GPR.py (for 10 variables)
...
def process_single_seed(seed_num, shared_data):
    """
    Run processing for a single seed (for parallel execution).
    """
    try:
        ...
        X_grid_2var = shared_data['X_grid_2var']
        X_act_2var = X_grid_2var.loc[selected_indices]
        D_2var = len(X_act_2var.T)

        a_mean, b_mean = sp.cal_coef(X_act_2var, y_act.iloc[:,0])
        a_sd = np.full(D_2var, 4)
        b_sd = 4
        ...
        for count in range(0,count_num):
            ...
            X_act_2var, _ = sp.add_act_data_BO(y_act, X_act_2var, EI, X_grid_2var, y_grid)
            ...
            if y_target.iloc[0,0] - y_act.iloc[-1,0] == 0:
                print(f"Process {os.getpid()}: Seed {seed_num}: finished")
...
def main():
    ...
    X_grid_2var = data_raw[["TPSA", "log(Weight)"]]
    X_grid_2var = X_grid_2var.reset_index(drop=True)
    ...
    for seed_num, success in results:
        if success:
            result_file = f_result / f"seed_{seed_num}" / "result.pickle"
            try:
                with open(result_file, mode='rb') as fi:
                    result_dict = pickle.load(fi)

                Y_end_all["target"][seed_num] = result_dict["y_target"]
                Y_end_all["initial"][seed_num] = result_dict["y_initial"]
                Y_end_all["experiment"][seed_num] = result_dict["y_experiment"]
                Y_end_all["initial_error"][seed_num] = result_dict["initial_error"]
                Y_end_all["experiment_error"][seed_num] = result_dict["experiment_error"]
            except FileNotFoundError:
                print(f"Result file not found for seed {seed_num}: {result_file}")
            except Exception as e:
                print(f"Error while loading result for seed {seed_num}: {str(e)}")
        else:
            print(f"Processing failed for seed {seed_num}")
