...
def process_single_seed(seed_num, shared_data):
    try:
        ...
        L_std_ave = pd.DataFrame()
        a_std_ave = pd.DataFrame()
        b_std_ave = pd.DataFrame()

        f_plot, f_hist, f_dens, f_trace = sp.make_folder_BO_GPR(seed_num, f_result)
        ...
        for count in range(0,count_num):
            ...
            for i, key in enumerate(["L","a","b"]):
                ...
                summary = arviz.summary(fit)  # When there is only one chain, r_hat becomes NaN.
                print(
            ...
            L_std = Y_pred["L"].std(0)
            a_std = Y_pred["a"].std(0)
            b_std = Y_pred["b"].std(0)
            ...
            L_std_ave.loc[count,0] = L_std.mean()
            a_std_ave.loc[count,0] = a_std.mean()
            b_std_ave.loc[count,0] = b_std.mean()
...
def main():
    ...
    sp.save_pca_contribution_ratio(pca_model, X_grid, Path("./result/BO(GPR)"))
    ...
    L_std_ave_all = pd.DataFrame()
    a_std_ave_all = pd.DataFrame()
    b_std_ave_all = pd.DataFrame()
    ...
    for seed_num, result in results:
        if result is not None:
            ...
            L_std_ave_all = pd.concat([L_std_ave_all, result["L_std_ave"]], axis=1)
            a_std_ave_all = pd.concat([a_std_ave_all, result["a_std_ave"]], axis=1)
            b_std_ave_all = pd.concat([b_std_ave_all, result["b_std_ave"]], axis=1)
        else:
